...
router.post('/', validate({body: require('../schema/offer_request_schema')}), function (req, res, next) {
    ...
    marketplaceCore.createOfferForRequest(req.query['userUUID'], req.accessToken, data, function (err, offer) {
        if (err) {
            next(err);
            return;
        }
...
router.get('/:id', function (req, res, next) {
    ...
    marketplaceCore.getOfferForId(req.query['userUUID'], req.accessToken, req.params['id'], function (err, offer) {
        if (err) {
            next(err);
            return;
        }
